...
class TestDataGraphIntegrity(unittest.TestCase):
    ...
    def test_node_count(self):
        ...
        self.assertEqual(actual_node_count, expected_node_count,
                         f"Node count mismatch in the data graph. Expected: {expected_node_count}, Actual: {actual_node_count}")

    def test_edge_count(self):
        expected_edge_count = 2
        actual_edge_count = self.graph.number_of_edges()
        self.assertEqual(actual_edge_count, expected_edge_count,
                         f"Edge count mismatch in the data graph. Expected: {expected_edge_count}, Actual: {actual_edge_count}")
    ...
    def test_query_result(self):
        # Test a query to find nodes with a price greater than 12
        expected_nodes = [2]
        query_nodes = [node for node, data in self.graph.nodes(data=True) if data["price"] > 12]
        self.assertEqual(query_nodes, expected_nodes, "Query result mismatch")
